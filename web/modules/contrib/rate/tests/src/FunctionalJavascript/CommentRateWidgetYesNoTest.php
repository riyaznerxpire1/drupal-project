 ...
 */
class CommentRateWidgetYesNoTest extends RateWidgetTestBase {
  ...
  public function testVoting(): void {
    ...
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.yesno-rating-label-yes');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
    ...
    $this->assertEquals('2 votes with an average rating of 1.', $vote_sum->getText());
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.yesno-rating-label-no');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
