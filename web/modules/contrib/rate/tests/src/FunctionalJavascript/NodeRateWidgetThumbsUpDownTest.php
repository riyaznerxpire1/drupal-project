 ...
 */
class NodeRateWidgetThumbsUpDownTest extends RateWidgetTestBase {
  ...
  public function testVoting() {
    ...
    // Voting.
    $page->find('css', 'label.thumbsupdown-rating-label-up')->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
    $vote_sum = $page->find('css', '.vote-result');
    ...
    $this->assertEquals('2 votes with an average rating of 1.', $vote_sum->getText());
    // Voting.
    $page->find('css', 'label.thumbsupdown-rating-label-down')->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
    $vote_sum = $page->find('css', '.vote-result');
