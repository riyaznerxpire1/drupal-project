 ...
 */
class CommentRateWidgetThumbsUpDownTest extends RateWidgetTestBase {
  ...
  protected function setUp(): void {
    ...
    $this->createRateWidget('thumbs_up_down', 'Thumbs Up / Down', 'thumbsupdown', $options, [], ['node.article'], [], [], $results);
  ...
  public function testVoting(): void {
    ...
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.thumbsupdown-rating-label-up');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
    ...
    $this->assertEquals('2 votes with an average rating of 1.', $vote_sum->getText());
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.thumbsupdown-rating-label-down');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
