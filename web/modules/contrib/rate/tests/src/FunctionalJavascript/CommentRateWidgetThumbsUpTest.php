 ...
 */
class CommentRateWidgetThumbsUpTest extends RateWidgetTestBase {
  ...
  protected function setUp(): void {
    ...
    $this->createRateWidget('thumbs_up', 'Thumbs Up', 'thumbsup', $options, [], ['node.article'], [], [], $results);
  ...
  public function testVoting(): void {
    ...
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.thumbsup-rating-label-up');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
