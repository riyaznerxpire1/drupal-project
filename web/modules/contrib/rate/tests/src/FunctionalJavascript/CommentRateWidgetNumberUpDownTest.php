 ...
 */
class CommentRateWidgetNumberUpDownTest extends RateWidgetTestBase {
  ...
  /**
   * Tests comment rate widget.
   *
   * Votes: +++-
   */
  public function testCommentRateWidget1(): void {
    ...
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.numberupdown-rating-label-up');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
    ...
    $this->assertEquals('3 votes with an average rating of 1.', $vote_sum->getText());
    // Voting.
    $comment_link = $this->assertSession()->elementExists('xpath', "//article[@id='comment-2']")->find('css', 'label.numberupdown-rating-label-down');
    $comment_link->click();
    // Reload page.
    drupal_flush_all_caches();
    $this->drupalGet('node/' . $this->node1->id());
  ...
  /**
   * Tests comment rate widget. Only -1.
   *
   * Votes: -
   */
  public function testCommentRateWidget2(): void {
    $comment = Comment::create([
      'entity_type' => 'node',
      'subject' => 'User 1 comment',
      'entity_id' => $this->node1->id(),
